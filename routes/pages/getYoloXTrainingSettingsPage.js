...
async function getYoloXInferencePage(req, res) {
    ...
    if (queries.project && typeof queries.project.getAllClasses === "function") {
        try {
            ...
            if (classRes) {
                classesAttempted = true;
                classes = Array.isArray(classRes.rows) ? classRes.rows : (Array.isArray(classRes) ? classRes : []);
            }
        } catch (err) {}
    }
    if (!classesAttempted && global.sqlite3) {
        try {
    ...
    const countFn = queries.project && (queries.project.getClassImageCounts || queries.project.getClassLabelCounts);
    if (typeof countFn === "function") {
        try {
            const countsResult = await countFn(projectDir);
            if (countsResult && countsResult.rows) {
                countsResult.rows.forEach((row) => {
                    classLabelCounts[row.CName] = row.imageCount !== undefined ? row.imageCount : (row.labelCount || 0);
                });
            }
        } catch (err) {
            if (global.logger && global.logger.error) global.logger.error(err);
        }
    }
