...
module.exports = {
	...
	getServerStatsPage: async (req, res) => {
		...
		var top_stdout, nvidia_smi, gpu_info = [];
		...
		try {
			...
			const { stdout } = await exec("nvidia-smi --format=csv,noheader,nounits --query-gpu=name,temperature.gpu,power.draw,power.limit,memory.used,memory.total,utilization.gpu");
			...
			gpu_info = lines.map(line => {
				let [name, temp, power_usage, power_cap, memory_used, memory_total, utilization] = line.split(",").map(x => x.trim());
				return {
					name,
					temp: `${temp} C`,
	...
	getProcessingPage: async (req, res) => {
		...
		for (var i = 0; i < runs.length; i++) {
			...
			if (err_idx >= 0) {
				...
				for (var j = 0; j < logs.length; j++) {
					...
					if (`${logs[j]}` == "datatovalues.py" || `${logs[j]}` == "images" || `${logs[j]}` == "labels" || `${logs[j]}` == "train" || `${logs[j]}` == "weights") {
						continue;
					}
					// weight.push(`${run_path}${logs[j]}`);
					weight.push(`${logs[j]}`);
		...
		for (var i = 0; i < inf_runs.length; i++) {
			...
			log_contents_inf.push(fs.readFileSync(`${run_path_inf}${inf_runs[i]}.log`, 'utf8'));
			...
			if (err_idx_inf >= 0) {
				...
				err_inf.push(fs.readFileSync(run_path_inf + logs_inf[err_idx_inf], 'utf8'));
	...
	getYoloXSettingsPage: async (req, res) => {
		...
		// This places the entire file into memory
		// Will not work with large files, apx 10000000 lines
		var paths = fs.readFileSync(yolovx_path_file, 'utf-8').split('\n').filter(Boolean);
