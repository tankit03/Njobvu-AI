...
const getImageSettingsPage = require("./pages/getImageSettingsPage");
...
const getCreatePage = require("./pages/getCreatePage");
const getAnnotatePage = require("./pages/getAnnotatePage");
const getProjectPage = require("./pages/getProjectPage");
const getValidationProjectPage = require("./pages/getValidationProjectPage");
const getConfigPage = require("./pages/getConfigPage");
...
<<<<<<< Updated upstream
    ...
    getClassificationPage: async (req, res) => {
    ...
    getAnnotatePage: async (req, res) => {
        ...
        var complete = Math.trunc(
            100 * (results6.length / results5["COUNT(*)"]),
        );
    ...
    getValidationProjectPage: async (req, res) => {
        ...
        if (
            ...
            confidenceImages.forEach((item) => {
                ...
                if (
                    !(IName in highestConf) ||
                    Confidence > highestConf[IName]
                ) {
    ...
    getLabelingPage: async (req, res) => {
        ...
        if (!fs.existsSync(abs_image_path)) {
            ...
            var rel_image_path =
                rel_project_path + "/images/" + results4[0].IName;
    ...
    getTrainingPage: async (req, res) => {
        ...
        for (var i = 0; i < runs.length; i++) {
            ...
            log_contents.push(
                fs.readFileSync(`${run_path}${runs[i]}.log`, "utf8"),
            );
    ...
    getServerStatsPage: async (req, res) => {
    ...
    getProcessingPage: async (req, res) => {
        ...
        for (var i = 0; i < inf_runs.length; i++) {
            ...
            if (err_idx_inf >= 0) {
                ...
                err_inf.push(
                    fs.readFileSync(
                        run_path_inf + logs_inf[err_idx_inf],
                        "utf8",
                    ),
                );
    ...
    getYoloXSettingsPage: async (req, res) => {
        const readdir = util.promisify(fs.readdir);
        const readFile = util.promisify(fs.readFile);
        ...
        res.render("training/yolovXSettings", {
            title: "yolovXSettings",
            user: req.cookies.Username,
            access: access,
            PName: PName,
            Admin: admin,
    ...
    getServerInfoPage: async (req, res) => {
        var IDX = parseInt(req.query.IDX),
            user = req.cookies.Username;
    ...
    getYolo3SettingsPage: async (req, res) => {
        const readdir = util.promisify(fs.readdir);
        const readFile = util.promisify(fs.readFile);
        ...
        // Get default python path
        var default_path = configFile.default_darknet_path;
        if (!default_path) {
            default_path = null;
        }

        res.render("training/yolov3Settings", {
            title: "yolov3Settings",
            user: req.cookies.Username,
            access: access,
            PName: PName,
            Admin: admin,
...
=======
    ...
    getProjectSettingsPage: getProjectSettingsPage,
    getClassSettingsPage: getClassSettingsPage,
    ...
    getClassificationPage: getClassificationPage,
    getCreatePage: getCreatePage,
    getAnnotatePage: getAnnotatePage,
    getReviewPage: getReviewPage,
    getProjectPage: getProjectPage,
    getValidationProjectPage: getValidationProjectPage,
